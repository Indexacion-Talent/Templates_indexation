...
(function () {
    var out = {};
    var html_jobs = document.querySelectorAll("#block-gavias-mion-content > div > article > div > div > div > div > div > div.content");
    var jobs = [];
    for (var x in html_jobs) {
        ...
        elem.querySelectorAll("#block-gavias-mion-content > div > article > div > div > div > div > div > div.content > p").forEach(e => {
            if (e.textContent.trim().includes("Location:")) {
                job.title = elem.querySelector("h2").textContent.trim();
                job.url = window.location.href;
                job.location = e.textContent.trim();
