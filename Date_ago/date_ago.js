job.dateposted_raw = elem.querySelector("a > div > div.vac_title__top > span").textContent.trim();
...
function ValidateLocation(ciudad) {
    var city;
    if (ciudad.includes("Available in")) {
        if (document.querySelector("div.information > p:nth-child(3) > span:nth-child(2) > span > button")) {
            var selectorBTNcity = document.querySelector("div.information > p:nth-child(3) > span:nth-child(2) > span > button").click();
            setTimeout(function () {
                var selectorCities = document.querySelector("ul.location-list.au-target").textContent.trim();
                if (selectorCities) {
                    city = selectorCities.replace(/\s+/gi, ' ');
                    msg("multiples ciudades " + city);
                }
            }, 500);
        }
    } else {
